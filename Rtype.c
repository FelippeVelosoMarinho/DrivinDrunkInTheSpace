...
void initChefao(Chefao *chefao)
{
	...
	chefao->vida = 1;
	chefao->chefe = al_load_bitmap("Img\\polvo2.png");
}
		...
		if (ev.type == ALLEGRO_EVENT_TIMER)
		{
			...
			if (menuPlay)
			{
				...
				while (noMenu)
				{
					...
					{ // Verifica se o botão "X" da janela foi pressionado e fecha o jogo
						playing = 0;
						break;
					}
				}
		...
		if (meteoro[i].vida)
		{
			for (int j = 0; j < NUM_METEORO; j++)
			{
				if (meteoro[j].vida )
				{
					if (i != j)
					{
						if (meteoro[i].x + meteoro[j].size >= meteoro[j].x - meteoro[i].size &&
							meteoro[i].x - meteoro[j].size <= meteoro[j].x + meteoro[i].size &&
							meteoro[i].y + meteoro[j].size >= meteoro[j].y - meteoro[i].size &&
							meteoro[i].y - meteoro[j].size <= meteoro[j].y + meteoro[i].size)
						{
							//printf("bleu");
							meteoro[i].vida = false;
...
{ // distancia pra acionar o modo de batalha
